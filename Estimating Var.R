...
n225 <- read.csv("~/ICA-Main/N225.csv", stringsAsFactors=FALSE)
fchi <- read.csv("~/ICA-Main/FCHI.csv", stringsAsFactors=FALSE)
...
#we conclude by fitting an ar(3)garch(1,1) model n225, and garch(1,1) to model fchi
model1=garchFit(formula=~arma(3,0)+garch(1,1),data=returnsDataSet$n225_returns,trace=F,cond.dist="sstd")
model2=garchFit(formula=~garch(1,1),data=returnsDataSet$fchi_returns,trace=F,cond.dist="sstd")
...
sim_values1 = (model1coef["mu"]+model1coef["ar1"]*tail[3] +model1coef["ar2"]*tail[2] +model1coef["ar3"]*tail[1])+sqrt(sigmaSquaredHat1)*res1_sim
...
sim_values2 = (model2coef["mu"]+sqrt(sigmaSquaredHat2)*res2_sim)
...
fit2 <- fitdistr(sim_values2, densfun = "normal")
#check if these normals fit their respective histograms
par(mfrow=c(2,1))
# Generate x values for plotting
hist2_x <- seq(min(sim_values1, sim_values2), max(sim_values1, sim_values2), length.out = 100)

# Plot histograms
par(mfrow = c(2, 1))
hist(sim_values1, prob = TRUE, main = "Histogram of sim_values1", breaks=50)
lines(hist2_x, dnorm(hist2_x, mean = fit1$estimate[1], sd = fit1$estimate[2]), col = "red")

hist(sim_values2, prob = TRUE, main = "Histogram of sim_values2", breaks =50)
lines(hist2_x, dnorm(hist2_x, mean = fit2$estimate[1], sd = fit2$estimate[2]), col = "blue")
