...
for (k in 1:6){  
  model1=garchFit(formula=~arma(3,0)+garch(1,1),data=returnsDataSet$fchi_returns,trace=F,cond.dist=distributions[k])
...
model2=garchFit(formula=~arma(3,0)+garch(1,1),data=returnsDataSet$fchi_returns,trace=F,cond.dist="sstd")
